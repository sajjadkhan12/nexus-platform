        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="mb-8">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold text-white mb-2">Provisioning Job</h1>
                        <p className="text-gray-400 font-mono text-sm">{job.id}</p>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 space-y-6">
                    ...
                    <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <div className="text-gray-200 font-medium">{job.triggered_by}</div>
                            </div>
                            <div>
                        ...
                        <div className="space-y-3">
                            {Object.entries(job.inputs).map(([key, value]) => (
                                <div key={key} className="group">
                                    <label className="block text-xs uppercase tracking-wider text-gray-500 mb-1 group-hover:text-indigo-400 transition-colors">
                                        {key.replace(/_/g, ' ')}
                                    </label>
                                    <div className="text-sm text-gray-300 font-mono bg-black/50 p-2 rounded border border-gray-800 break-all">
                                        {String(value)}
                                    </div>
                                </div>
                            ))}
                    ...
                    {job.outputs && Object.keys(job.outputs).length > 0 && (
                        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                            <h3 className="text-lg font-semibold text-white mb-4">Outputs</h3>
                            <div className="space-y-3">
                                {Object.entries(job.outputs).map(([key, value]) => (
                                    <div key={key} className="group">
                                        <label className="block text-xs uppercase tracking-wider text-gray-500 mb-1 group-hover:text-green-400 transition-colors">
                                            {key}
                                        </label>
                                        <div className="text-sm text-green-400 font-mono bg-green-900/10 p-2 rounded border border-green-900/20 break-all">
                                            {String(value)}
                                        </div>
                                    </div>
                                ))}
                ...
                <div className="lg:col-span-2">
                    <div className="bg-gray-900 border border-gray-800 rounded-xl flex flex-col h-[600px]">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 font-mono text-sm space-y-2 bg-black/50">
                            {logs.length === 0 ? (
                                ...
                                logs.map((log) => (
                                    <div key={log.id} className="flex items-start gap-3 hover:bg-white/5 p-1 rounded transition-colors">
                                        <span className="text-gray-600 text-xs whitespace-nowrap pt-0.5">
                                            {new Date(log.timestamp).toLocaleTimeString()}
                                        </span>
                                        <span className={`text-xs font-bold uppercase w-16 pt-0.5 ${getLevelColor(log.level)}`}>
                                            {log.level}
                                        </span>
                                        <span className="text-gray-300 break-words whitespace-pre-wrap flex-1">
                                            {log.message}
                                        </span>
                                    </div>
                                ))
